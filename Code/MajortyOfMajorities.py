...
def majority_of_majorities_jc(graph, ratings, rated_by, item):
    ...
    def _surrounding_majority(v):
        ...
        for o_v in graph[v]:
            if o_v in recommendations:
                jaccards[recommendations[o_v] - 1] += jaccard_index(graph, v, o_v)
    ...
    q = sorted(list(set(graph.keys()) - rb), key = lambda x: _out_voters(x), reverse=True)
    while len(q) != 0:
        current = q.pop(0)
...
def top_k_maj(k, *args):
    def _inner_func(graph, ratings, rated_by, item):
        ...
        for node in jaccards:
            sorted_jaccs = sorted(jaccards[node].items(), key=lambda x: x[1], reverse=True)[:k]
